...
/* Jinfan.Hu@ODM_WT.BSP.Kernel.Boot, 2018/12/7, Add for serial mode change */
...
static struct pinctrl_state *serial_urxd0 = NULL, *serial_gpio_rx = NULL;
static struct pinctrl_state *serial_utxd0 = NULL, *serial_gpio_tx = NULL;
#endif /*ODM_WT_EDIT*/
...
/* Jinfan.Hu@ODM_WT.BSP.Kernel.Boot, 2018/12/7, Add for serial mode change */
	if (get_boot_mode() == FACTORY_BOOT){
		pr_err("%s boot_mode is factory, skip change serial mode to gpio!\n", __func__);
	} else {
		serial_pinctrl = devm_pinctrl_get(&pdev->dev);
		if (IS_ERR(serial_pinctrl)) {
			err = PTR_ERR(serial_pinctrl);
			pr_err("%s can't find serial rx pinctrl!\n", __func__);
			return err;
		}

		serial_urxd0 = pinctrl_lookup_state(serial_pinctrl, "serial_urxd0");
		if (IS_ERR(serial_urxd0)) {
			err = PTR_ERR(serial_urxd0);
			pr_err("%s can't find serial pinctrl serial_urxd0!\n", __func__);
			return err;
		}

		serial_gpio_rx = pinctrl_lookup_state(serial_pinctrl, "serial_gpio_rx");
		if (IS_ERR(serial_gpio_rx)) {
			err = PTR_ERR(serial_gpio_rx);
			pr_err("%s can't find serial pinctrl serial_gpio_rx!\n", __func__);
			return err;
		}

		err = pinctrl_select_state(serial_pinctrl, serial_gpio_rx);
		if(err) {
			pr_err("%s change serial rx mode to gpio fail\n", __func__);
		} else {
			pr_err("%s change serial rx mode to gpio success\n", __func__);
		}


		serial_utxd0 = pinctrl_lookup_state(serial_pinctrl, "serial_utxd0");
		if (IS_ERR(serial_utxd0)) {
			err = PTR_ERR(serial_utxd0);
			pr_err("%s can't find serial pinctrl serial_utxd0!\n", __func__);
			return err;
		}

		serial_gpio_tx = pinctrl_lookup_state(serial_pinctrl, "serial_gpio_tx");
		if (IS_ERR(serial_gpio_tx)) {
			err = PTR_ERR(serial_gpio_tx);
			pr_err("%s can't find serial pinctrl serial_gpio_tx!\n", __func__);
			return err;
		}

		err = pinctrl_select_state(serial_pinctrl, serial_gpio_tx);
		if(err) {
			pr_err("%s change serial tx mode to gpio fail\n", __func__);
		} else {
			pr_err("%s change serial tx mode to gpio success\n", __func__);
		}
	}
#endif /*ODM_WT_EDIT*/
