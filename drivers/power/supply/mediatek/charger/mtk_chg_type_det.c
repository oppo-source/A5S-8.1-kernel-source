...
extern bool g_otg_online_value ;
#endif /*ODM_WT_EDIT*/
...
//Yanchao.Hu@ODM_WT.BSP.Storage.otg, 2018/11/16, Modify for summer otg_switch
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		val->intval = mtk_chg->s_otg_switch;
		break;
//Yanchao.Hu@ODM_WT.BSP.Storage.otg, 2018/11/20, Modify for summer otg_online
	case POWER_SUPPLY_PROP_OTG_ONLINE:
		val->intval = g_otg_online_value;
		break;
#endif /*ODM_WT_EDIT*/
	default:
		return -EINVAL;
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_OTG_SWITCH:
		if (mtk_chg->s_otg_switch != !!val->intval) {
			mtk_chg->s_otg_switch = !!val->intval;
			musb_ctrl_host(mtk_chg->s_otg_switch);
		}
		break;
	default:
		return -EINVAL;
...
//Yanchao.Hu@ODM_WT.BSP.Storage.otg, 2018/11/16, Modify for summer otg_switch
	...
	mt_chg->usb_desc.property_is_writeable = mt_usb_prop_is_writeable;
	mt_chg->s_otg_switch = false; //default state
#endif /*ODM_WT_EDIT*/
