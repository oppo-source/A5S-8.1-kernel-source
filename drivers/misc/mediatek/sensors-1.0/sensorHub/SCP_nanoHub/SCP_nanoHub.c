...
// Jianfeng.Liang@ODM_WT.BSP.Sensors.Config, 2018/11/17, Add for engmode sensors
		case CUST_ACTION_SET_CALI:
			...
			req.set_cust_req.setCali.int32_data[0] =
				*((int32_t *) data);
			req.set_cust_req.setCali.int32_data[1] =
				*((int32_t *) data + 1);
			len = offsetof(SCP_SENSOR_HUB_SET_CUST_REQ, custData)
			    + sizeof(req.set_cust_req.setCali);
			break;
...
// Jianfeng.Liang@ODM_WT.BSP.Sensors.Config, 2018/11/17, Add for engmode sensors
		case CUST_ACTION_SELFTEST:
			req.set_cust_req.selftest.action = CUST_ACTION_SELFTEST;
			len = offsetof(SCP_SENSOR_HUB_SET_CUST_REQ, selftest)
			...
			if (err == 0) {
				if ((req.set_cust_rsp.action !=
					SENSOR_HUB_SET_CUST)
					|| (req.set_cust_rsp.errCode != 0)) {
					pr_err("scp_sHub_req_send CUST_ACTION_SELFTEST fail 1!\n");
					return -1;
				}
				if (req.set_cust_rsp.selftest.action !=
					CUST_ACTION_SELFTEST) {
					pr_err("scp_sHub_req_send CUST_ACTION_SELFTEST fail 2!\n");
					return -1;
				}
				*((uint32_t *) data) = req.set_cust_rsp.selftest.testResult;
			} else {
