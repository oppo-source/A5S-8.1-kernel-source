...
#ifdef ODM_WT_EDIT
// Jianfeng.Liang@ODM_WT.BSP.Sensors.Config, 2018/11/07, Add for engmode sensors
static DRIVER_ATTR(selftest, 0644, show_selftest_value, NULL);
#endif
...
#ifdef ODM_WT_EDIT
// Jianfeng.Liang@ODM_WT.BSP.Sensors.Config, 2019/03/01, Modify for OPPO Gsensor calibration
bool oppo_gsensor_cali_enable = false;
...
static int gsensor_factory_set_cali(int32_t data[3])
{
	...
	sensor_calibration_to_hub(ID_ACCELEROMETER);
#endif
	return 0;
}
#endif
...
#ifdef ODM_WT_EDIT
// Jianfeng.Liang@ODM_WT.BSP.Sensors.Config, 2019/03/01, Modify for OPPO Gsensor calibration
	pr_debug("gsensor_factory_get_cali: (%d, %d, %d)!\n", data[0], data[1], data[2]);
	if (status != 0) {
		pr_debug("gsensor static cali detect shake!\n");
		if(oppo_gsensor_cali_enable)
		{
			oppo_gsensor_cali_enable = false;
			if(((data[0] > -980) && (data[0] < 980)) && ((data[1] > -980) && (data[1] < 980)) && ((data[2] > -980) && (data[2] < 980))) {
				status = obj->static_cali_status = 0;
			}
			else
				return -2;
		}
