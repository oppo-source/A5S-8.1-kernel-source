...
static kal_uint16 gc8034_table_write_cmos_sensor(
...
static void gc8034_gcore_read_otp_info(void)
{
	...
	pr_debug("GC8034_OTP : flag_module = 0x%x , flag_wb = 0x%x , flag_af = 0x%x\n", flag_module, flag_wb, flag_af);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_debug("GC8034_OTP_INFO group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				...
				pr_err("GC8034_OTP_INFO Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_debug("GC8034_OTP_WB group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				...
				pr_err("GC8034_OTP_WB Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
		...
		case 0x00:
			...
			break;
		case 0x40:
			pr_debug("GC8034_OTP_GOLDEN group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				...
				pr_err("GC8034_OTP_GOLDEN Check sum %d Error !!\n", index + 1);
		...
		case 0x00:
			pr_err("GC8034_OTP_AF group %d is Empty !!\n", index + 1);
			gc8034_otp_info.af_flag = gc8034_otp_info.af_flag | 0x00;
			break;
		case 0x04:
			pr_debug("GC8034_OTP_AF group %d is Valid !!\n", index + 1);
			check = 0;
			...
			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				...
				pr_err("GC8034_OTP_AF Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			pr_err("GC8034_OTP_AF group %d is Invalid !!\n", index + 1);
	...
	pr_debug("GC8034_OTP_LSC : flag_lsc = 0x%x\n", flag_lsc);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			...
			break;
		case 0x04:
			pr_debug("GC8034_OTP_LSC group %d is Valid !\n", index + 1);
			check = 0;
			if (index == 0) {
			...
			if ((check % 255 + 1) == lsc_checksum)
				...
				pr_err("GC8034_OTP_LSC check sum error, check sum read : 0x%x, calculate:0x%x!!\n", lsc_checksum, check % 255 + 1);
			}
			break;
		case 0x08:
		case 0x0c:
	...
	pr_debug("GC8034_OTP_INFO:module_id=0x%x\n", gc8034_otp_info.module_id);
	pr_debug("GC8034_OTP_INFO:lens_id=0x%x\n", gc8034_otp_info.lens_id);
	pr_debug("GC8034_OTP_INFO:vcm_id=0x%x\n", gc8034_otp_info.vcm_id);
	pr_debug("GC8034_OTP_INFO:vcm_driver_id=0x%x\n", gc8034_otp_info.vcm_driver_id);
	pr_debug("GC8034_OTP_INFO:data=%d-%d-%d\n", gc8034_otp_info.year, gc8034_otp_info.month, gc8034_otp_info.day);
	pr_debug("GC8034_OTP_WB:r/g=0x%x\n", gc8034_otp_info.rg_gain);
	pr_debug("GC8034_OTP_WB:b/g=0x%x\n", gc8034_otp_info.bg_gain);
	pr_debug("GC8034_OTP_GOLDEN:golden_rg=0x%x\n", gc8034_otp_info.golden_rg);
	pr_debug("GC8034_OTP_GOLDEN:golden_bg=0x%x\n", gc8034_otp_info.golden_bg);
	pr_debug("GC8034_OTP_AF:infitiy=0x%x\n", gc8034_otp_info.af_infinity);
	pr_debug("GC8034_OTP_AF:macro=0x%x\n", gc8034_otp_info.af_macro);
#endif
	...
	for (i = 0; i < 10; i++) {
		...
		for (j = 0; j < 128; j++)
			pr_debug("GC8034_OTP_DEBUG: Page%d: addr = 0x%x, value = 0x%x;\n", i, j, debug[j]);
	}
#endif
}
...
static void gc8034_gcore_update_dd(void)
{
	...
	if (gc8034_otp_info.dd_flag == 0x01) {
		...
		write_cmos_sensor(0xfe, 0x00);
        LOG_INF("GC8034_OTP_AUTO_DD End !\n");
	}
}
...
static void gc8034_gcore_update_wb(void)
{
	...
	if (0x01 == (gc8034_otp_info.golden_flag & 0x01)) {
		...
		pr_debug("GC8034_OTP_UPDATE_AWB:rg_typical = 0x%x, bg_typical = 0x%x\n", rg_typical, bg_typical);
	} else {
	...
	if (0x01 == (gc8034_otp_info.wb_flag & 0x01)) {
		...
		pr_debug("GC8034_OTP_UPDATE_AWB:r = 0x%x, g = 0x%x, b = 0x%x\n", r_gain, g_gain, b_gain);
...
static void gc8034_gcore_read_otp(void)
{
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_gc8034[] = {
	0xf2, 0x00,
	0xf4, 0x80,
	0xf5, 0x19,
	0xf6, 0x44,
	...
	0xc3, 0xff,
	0x90, 0x01,
	0x92, GC8034_BinStartY,
	0x94, GC8034_BinStartX,
	0x95, 0x04,
	0x96, 0xc8,
	0x97, 0x06,
	0x98, 0x60,
...
static void sensor_init(void)
{
	...
	gc8034_table_write_cmos_sensor(
		addr_data_pair_init_gc8034,
		sizeof(addr_data_pair_init_gc8034) / sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_binning_gc8034[] = {
	0xf2, 0x00,
	0xf4, 0x80,
	0xf5, 0x19,
	0xf6, 0x44,
...
{
#if MULTI_WRITE
	LOG_INF("binning_setting multi write\n");
	gc8034_table_write_cmos_sensor(
		addr_data_pair_binning_gc8034,
		sizeof(addr_data_pair_binning_gc8034) / sizeof(kal_uint16));
#else
	LOG_INF("binning_setting normal write\n");
	/* SYS */
	write_cmos_sensor(0xf2, 0x00);
	write_cmos_sensor(0xf4, 0x80);
	write_cmos_sensor(0xf5, 0x19);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_fullsize_gc8034[] = {
	0xf2, 0x00,
	0xf4, 0x80,
	0xf5, 0x19,
	0xf6, 0x44,
...
{
#if MULTI_WRITE
	LOG_INF("fullsize_setting multi write\n");
	gc8034_table_write_cmos_sensor(
		addr_data_pair_fullsize_gc8034,
		sizeof(addr_data_pair_fullsize_gc8034) / sizeof(kal_uint16));
#else
	LOG_INF("fullsize_setting normal write\n");
	/* SYS */
	write_cmos_sensor(0xf2, 0x00);
	write_cmos_sensor(0xf4, 0x80);
	write_cmos_sensor(0xf5, 0x19);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_gc8034[] = {
	0xf2, 0x00,
	0xf4, 0x80,
	0xf5, 0x19,
	0xf6, 0x44,
...
static void slim_video_setting(void)
{
	...
	gc8034_table_write_cmos_sensor(
		addr_data_pair_slim_video_gc8034,
		sizeof(addr_data_pair_slim_video_gc8034) / sizeof(kal_uint16));
#else
	LOG_INF("slim_video_setting multi write\n");
	/* SYS */
	write_cmos_sensor(0xf2, 0x00);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_info("GC8034_I2CBUS = %d, I2C_bus = %d\n", GC8034_I2CBUS, I2C_bus);
	if (I2C_bus != GC8034_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		pr_err("GC8034_I2CBUS: %d, I2C_bus = %d, Check Error!\n", GC8034_I2CBUS, I2C_bus);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable){
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0x3f, 0xd0); /* stream on */
		write_cmos_sensor(0xfe, 0x00);
	} else {
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0x3f, 0x00); /* stream off */
		write_cmos_sensor(0xfe, 0x00);
	}
