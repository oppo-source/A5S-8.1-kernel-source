...
#include "gc2375hlianyimipiraw_Sensor.h"
...
extern kal_bool GC2375HDuringTestPattern;
static unsigned int DEPTH_CAM_GPIO_NUM=178;
#define VCAM_ID "camera,vcam_id"


static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2375H_LIANYI_SENSOR_ID, /* record sensor id defined in Kd_imgsensor.h */
...
static kal_uint32 return_sensor_id(void)
{
	...
	DEPTH_CAM_GPIO_NUM = of_get_named_gpio_flags(node, VCAM_ID, 0, &flag);
	if(gpio_request(DEPTH_CAM_GPIO_NUM , "vcam_id_pin") < 0)
	LOG_INF("gc2375h lianyi gpio_request gpio_VCAM_EN:%d error", DEPTH_CAM_GPIO_NUM);
	if (!gpio_is_valid(DEPTH_CAM_GPIO_NUM)) {
				LOG_INF("Invalid RESET gpio: %d\n", DEPTH_CAM_GPIO_NUM);
	}
	ret = gpio_direction_input(DEPTH_CAM_GPIO_NUM);
	LOG_INF("gpio_direction_input  %d\n", ret);
	ret_ID = gpio_get_value(DEPTH_CAM_GPIO_NUM);
	LOG_INF("gc2375h lianyi DEPTH_CAM_GPIO_NUM ret is %d",ret_ID);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	LOG_INF("GC2375H_I2CBUS = %d, I2C_bus = %d\n", GC2375H_I2CBUS, I2C_bus);
	if (I2C_bus != GC2375H_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		pr_err("GC2375H_I2CBUS: %d, I2C_bus = %d, Check Error!\n", GC2375H_I2CBUS, I2C_bus);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				hardwareinfo_set_prop(HARDWARE_BACK_SUB_CAM_MOUDULE_ID, "lianyi");
