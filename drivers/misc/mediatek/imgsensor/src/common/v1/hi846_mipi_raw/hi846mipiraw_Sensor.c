...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data ={
	...
	.deviceID = 0x02,
	.dataLength = 1886,
	.sensorVendorid = 0x19020000, /* MoudleID+positionID*/
	.vendorByte = {1},
	.dataBuffer = NULL,
};
...
static int hi846_sensor_otp_info(struct stCAM_CAL_DATAINFO_STRUCT*pData){
	int iCount = 0;
	int buf_id = 0;
    u32 flag = 0;
	u32 flag_address = 0;
    ...
    if(pData->dataBuffer[0] != (pData->sensorVendorid >> 24)){
		LOG_DBG("HI846_OTP_INFO:module_id=0x%x not match readed 0x%x\n",pData->sensorVendorid,pData->dataBuffer[0]);
		return -1;
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		size = hi846_sensor_otp_info(&sensor_eeprom_data);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "tongxingda");
