...
#include "hi846dongcimipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_DONGCI_SENSOR_ID,
...
static struct stCAM_CAL_DATAINFO_STRUCT sensor_eeprom_data ={
	.sensorID= HI846_DONGCI_SENSOR_ID,
	.deviceID = 0x02,
	.dataLength = 1886,
	.sensorVendorid = 0x13020000, /* MoudleID+positionID*/
	.vendorByte = {1},
	.dataBuffer = NULL,
};

static struct stCAM_CAL_CHECKSUM_STRUCT hi846Checksum[4] =
{
	{MODULE_ITEM,0x0000,0x0000,0x0006,0x0007,0x00},
	{AWB_ITEM,0x0008,0x0008,0x000F,0x0010,0x00},
	{LSC_ITEM,0x0011,0x0011,0x075C,0x075D,0x00},
	{MAX_ITEM,0xFFFF,0xFFFF,0xFFFF,0xFFFF,0x00},  // this line must haved
};
...
static int hi846_dongci_sensor_otp_info(struct stCAM_CAL_DATAINFO_STRUCT*pData){
	int iCount = 0;
	int buf_id = 0;
    u32 flag = 0;
	u32 flag_address = 0;
	u32 minfo_startaddr = 0x0201;
	u32 awb_startaddr = 0x021a;
	u32 lsc_startaddr = 0x0236;
	int minfo_width = 8;
	int awb_width = 9;
	int lsc_width = 1869;
	...
	/* 3. read eeprom data */
	/********************* minfo group **************************/
    flag = hi846_sensor_otp_read(minfo_startaddr);
    //LOG_DBG("%s:%d, minfo flag_address=0x%x\n", __func__, __LINE__, flag);
    if(0x37 == flag)
    {
        flag_address = 0x0212;
    } else if (0x13 == flag) {
        ...
        LOG_ERR("hi846 minfo flag failed! \n");
    }
    for(iCount = 0;((iCount < minfo_width)&&(buf_id < pData->dataLength));iCount++,buf_id++){
        pData->dataBuffer[buf_id] = (kal_uint8)hi846_sensor_otp_read(flag_address+iCount);
        //LOG_DBG("MINFO OTP[%d]= 0x%2x\n", buf_id, pData->dataBuffer[buf_id]);
    }

	LOG_DBG("HI846_OTP_INFO:module_id=0x%x\n", pData->dataBuffer[0]);
	LOG_DBG("HI846_OTP_INFO:position_id=0x%x\n", pData->dataBuffer[1]);
	LOG_DBG("HI846_OTP_INFO:data=%d-%d-%d\n", pData->dataBuffer[4]+2000, pData->dataBuffer[5], pData->dataBuffer[6]);
    if(pData->dataBuffer[0] != (pData->sensorVendorid >> 24)){
		LOG_DBG("HI846_DONGCI_OTP_INFO:module_id=0x%x not match readed 0x%x\n",pData->sensorVendorid,pData->dataBuffer[0]);
		return -1;
	}
	/********************* awb group **************************/
    flag = hi846_sensor_otp_read(awb_startaddr);
    //LOG_DBG("%s:%d, awb flag_address=0x%x\n", __func__, __LINE__, flag);
    if(0x37 == flag)
    {
        flag_address = 0x022D;
    } else if (0x13 == flag) {
        ...
        LOG_ERR("hi846 awb flag failed! \n");
        return -1;
    }
    for(iCount = 0;((iCount < awb_width)&&(buf_id < pData->dataLength));iCount++,buf_id++){
        pData->dataBuffer[buf_id] = (kal_uint8)hi846_sensor_otp_read(flag_address+iCount);
        //LOG_DBG("AWB OTP[%d]= 0x%2x\n", buf_id, pData->dataBuffer[buf_id]);
    }

	/********************* lsc group **************************/
    flag = hi846_sensor_otp_read(lsc_startaddr);
    //LOG_DBG("%s:%d, lsc flag_address=0x%x\n", __func__, __LINE__, flag);
    if(0x37 == flag)
    {
        flag_address = 0x10D1;
    } else if (0x13 == flag) {
	...
	/* 4. check eeprom data */
	if(imgSensorCheckEepromData(pData, hi846Checksum) != 0){
		LOG_ERR("hi846 OTP checksum failed! \n");
		hi846_disable_otp_func();
		kfree(pData->dataBuffer);
		pData->dataBuffer = NULL;
		return -1;
	} else {
        LOG_DBG("hi846 OTP checksum success!\n");
    }
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_info("HI846_I2CBUS = %d, I2C_bus = %d\n", HI846_I2CBUS, I2C_bus);
	if (I2C_bus != HI846_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		pr_err("HI846_I2CBUS: %d, I2C_bus = %d, Check Error!\n", HI846_I2CBUS, I2C_bus);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		size = hi846_dongci_sensor_otp_info(&sensor_eeprom_data);
