...
#ifdef ODM_WT_EDIT
/* Huijuan.Niu@ODM_WT.Network.RF.SWTP, 2018/11/28, add swtp gpio state */
...
{
	proc_create("swtp_gpio_value", 0444, NULL, &swtp_gpio_fops);
}
#endif /*ODM_WT_EDIT*/
...
int swtp_init(int md_id)
{
	int ret = 0;
	#ifndef ODM_WT_EDIT
	// huijuan.Niu@ODM_WT.NW.RF.SWTP.noBug, 2018/11/28, Add for SWTP featrue to set right GPIO
	u32 ints[2] = { 0, 0 };
	#else  /* ODM_WT_EDIT */
	u32 ints[1] = { 0 };
	#endif  /* ODM_WT_EDIT */
	...
	if (node) {
		...
		#else  /* ODM_WT_EDIT */
		ints1[0] = of_get_named_gpio(node, "deb-gpios",0);
		swtp_data[md_id].gpiopin = ints1[0];
		swtp_data[md_id].setdebounce = ints[0];
		swtp_data[md_id].eint_type = ints1[1];
		#endif  /* ODM_WT_EDIT */
		#ifndef ODM_WT_EDIT
		// huijuan.Niu@ODM_WT.NW.RF.SWTP.noBug, 2019/01/09, Add for SWTP featrue to set GPIO debounce
		gpio_set_debounce(swtp_data[md_id].gpiopin,
			swtp_data[md_id].setdebounce);
		#else  /* ODM_WT_EDIT */
		ret=gpio_set_debounce(swtp_data[md_id].gpiopin,
			swtp_data[md_id].setdebounce);
		#endif  /* ODM_WT_EDIT */
		#ifdef ODM_WT_EDIT
		// huijuan.Niu@ODM_WT.NW.RF.SWTP.noBug, 2019/01/09, Add for SWTP featrue to set GPIO debounce
		CCCI_LEGACY_ERR_LOG(md_id, SYS,
                "swtp GPIO=%d, setdebounce=%d, eint_type=%d,ret=%d\n",
