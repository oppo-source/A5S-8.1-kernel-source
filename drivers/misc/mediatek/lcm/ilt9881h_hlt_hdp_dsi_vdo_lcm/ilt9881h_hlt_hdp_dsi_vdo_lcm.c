...
** ODM_WT_EDIT
** File: - ilt9881h_hlt_hdp_dsi_vdo_lcm.c
** Description: source file for lcm ili9881h+hlt in kernel stage
**
** Version: 1.0
** Date : 2018/08/09
...
**  houbenzhong       2018/08/09     1.0     source file for lcm ili9881h+hlt in kernel stage
**
****************************************************************/
...
//Bin.Su@ODM_WT.BSP.TP.Timing.2018/11/01,Adjust TP timing in suspend and resume status
	...
	if (lcm_dsi_mode == CMD_MODE) {
		...
		pr_debug("ili9881h_hlt_lcm_mode = cmd mode :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("ili9881h_hlt_lcm_mode = vdo mode :%d----\n", lcm_dsi_mode);
	}
}
...
{
	pr_debug("[lcd] ili9881h_hlt_cabc_mode is %d \n", level);
	if (level) {
...
{
	pr_debug("ili9881h_hlt backlight_level = %d\n", level);
...
static unsigned int lcm_esd_recover(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		push_table(NULL, init_setting_cmd, sizeof(init_setting_cmd) / sizeof(struct LCM_setting_table), 1);
		pr_debug("ili9881h_hlt_lcm_mode = cmd mode esd recovery :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("ili9881h_hlt_lcm_mode = vdo mode esd recovery :%d----\n", lcm_dsi_mode);
	}
...
struct LCM_DRIVER ilt9881h_hlt_hdp_dsi_vdo_lcm_drv = {
	.name = "ilt9881h_hlt_hdp_dsi_vdo_lcm_drv",
