...
** ODM_WT_EDIT
** File: - nt36525b_skyw_hdp_dsi_vdo_lcm.c
** Description: source file for lcm nt36525b+skyw in kernel stage
**
** Version: 1.0
** Date : 2018/12/03
...
**  houbenzhong       2018/12/03    1.0     source file for lcm nt36525b+skyw in kernel stage
**
****************************************************************/
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_frontporch = 109;
...
//Bin.Su@ODM_WT.BSP.TP.Timing.2018/11/01,Adjust TP timing in suspend and resume status
	...
	if (lcm_dsi_mode == CMD_MODE) {
		...
		pr_debug("nt36525b_skyw_lcm_mode = cmd mode :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("nt36525b_skyw_lcm_mode = vdo mode :%d----\n", lcm_dsi_mode);
	}
}
...
{
	pr_debug("[lcd] nt36525b_skyw_cabc_mode is %d \n", level);
	if (level) {
...
{
	pr_debug("nt36525b_skyw backlight_level = %d\n", level);
...
static unsigned int lcm_esd_recover(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		push_table(NULL, init_setting_cmd, sizeof(init_setting_cmd) / sizeof(struct LCM_setting_table), 1);
		pr_debug("nt36525b_skyw_lcm_mode = cmd mode esd recovery :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		pr_debug("nt36525b_skyw_lcm_mode = vdo mode esd recovery :%d----\n", lcm_dsi_mode);
	}
...
struct LCM_DRIVER nt36525b_skyw_hdp_dsi_vdo_lcm_drv = {
	.name = "nt36525b_skyw_hdp_dsi_vdo_lcm_drv",
