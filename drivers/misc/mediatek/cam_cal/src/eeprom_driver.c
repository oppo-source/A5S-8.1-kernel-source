...
extern unsigned int cam_cal_get_func_list(struct stCAM_CAL_FUNC_STRUCT **ppCamcalFuncList);
	...
	if (pCamCalList != NULL&& pCamCalFunc != NULL) {
		...
		for (i = 0; pCamCalList[i].sensorID != 0; i++) {
			if (pCamCalList[i].sensorID == sensorID) {
				...
				if(pCamCalList[i].sensorID == S5K3P9SP_SENSOR_ID){
...
void dumpEEPROMData(int u4Length,u8* pu1Params, int deviceid)
{
    ...
    PK_INF("oppo ######################### start dump eeprom data, deviceID=%d #################\n", deviceid);
	...
	PK_INF("\n");
    PK_INF("oppo ######################### end dump eeprom data, deviceID=%d #################\n", deviceid);
#endif
}
		...
		for((count = 0);count < MAX_ITEM;count++){
			if((cData[count].item < MAX_ITEM)){
				...
				if(((sum%0xff)+1)!= buffer[cData[count].checksumAdress]) {
					PK_ERR("checksum cItem=%d,0x%x,length = 0x%x failed(saddr=0x%x, eaddr=0x%x)!\n",cData[count].item,sum,length,
	...
	if (pcmdInf != NULL) {
		if (pcmdInf->readCMDFunc != NULL){
			...
			if(i4RetValue != pData->dataLength) {
				...
				if(pData->sensorID == S5K3L6_SENSOR_ID) {
				   efuseCount = 11;
				}else if(pData->sensorID == HI1336_SENSOR_ID) {
				   efuseCount = 11;
				}else if(pData->sensorID == HI1336_LIANYI_SENSOR_ID) {
				   efuseCount = 11;
				}
	         ...
	         const char *buf, size_t count)
{
	...
	if(sensorID == S5K3L6_SENSOR_ID){
		startAdress = 0x13B5;
	}
	else if(sensorID == HI1336_SENSOR_ID){
		startAdress = 0x13E4;
	}else{
