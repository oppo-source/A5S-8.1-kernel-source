...
/*  VCC_SUPPLY SELECT add by johnson  */
#define VCC_SUPPLY_USE_REGULATER
//#define VCC_SUPPLY_USE_GPIO
//#define VCC_SUPPLY_USE_PINCTRL
...
#ifdef PROC_NODE
static void egistec_proc_create_node(struct egistec_data *data);
static int egistec_proc_init(struct egistec_data *data);
#endif
...
static int egistec_power_on_off(struct egistec_data *egistec)
{
#ifdef VCC_SUPPLY_USE_REGULATER
	int ldo_vout_status;
	ldo_vout_status = regulator_disable(egistec_regulater);
	if (ldo_vout_status < 0) {
		printk("egitec regulator_disable fail \n");
		return -1;
	}
...
#ifdef VCC_SUPPLY_USE_PINCTRL
	DEBUG_PRINT("%s\n", __func__);
	pinctrl_select_state(egistec->pinctrl_gpios, egistec->pins_power_off);
	mdelay(15);
	pinctrl_select_state(egistec->pinctrl_gpios, egistec->pins_power_on);
	printk("egistec pinctrl_power_off_on success \n");
#endif
...
static long egistec_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	break;
	case FP_RESET_REQUEST:
		DEBUG_PRINT("fp_ioctl ioc->opcode == FP_RESET_REQUEST ++");
		if (spi_register_driver(&spi_driver))
			{
				pr_err("egistec_module_init7");
				printk(KERN_ERR "register spi driver fail %s\n", __func__);
				return -EINVAL;
			}
		DEBUG_PRINT("fp_ioctl ioc->opcode == FP_RESET_REQUEST --");
		retval = egistec_parse_dt(egistec);
		goto done;
	...
	case FP_POWER_ONOFF:
			...
			egistec_power_on_off(egistec);
		goto done;
	case FP_PROC_NODE:
			DEBUG_PRINT("fp_ioctl <<< FP_PROC_NODE    -------  \n");
			egistec_proc_create_node(egistec);
		goto done;
	case FP_SCREEN_ONOFF:
...
//	struct device_node *np = dev->of_node;
	...
	printk(KERN_ERR "%p egistec enter 1\n", node);
	printk(KERN_ERR "%s egistec enter 0\n", __func__);

	if (node) {
		printk(KERN_ERR "%s egistec enter 1\n", __func__);
		pdev = of_find_device_by_node(node);
		//printk(KERN_ERR "%s egistec enter 1\n", *pdev);
		printk(KERN_ERR "%s egistec enter 2\n", __func__);
		if (pdev) {
			
			printk(KERN_ERR "%s egistec enter 3\n", __func__);
			...
			pr_err("weidong reset1");
			pinctrl_select_state(data->pinctrl_gpios, data->pins_reset_high);
			pr_err("weidong reset2");
#ifdef VCC_SUPPLY_USE_PINCTRL
			data->pins_power_on = pinctrl_lookup_state(data->pinctrl_gpios, "power_on");
			if (IS_ERR(data->pins_power_on)) {
				ret = PTR_ERR(data->pins_power_on);
				devm_pinctrl_put(data->pinctrl_gpios);
				printk(KERN_ERR "%s can't find fingerprint pinctrl pins_power_on\n", __func__);
				return ret;
			}
			data->pins_power_off = pinctrl_lookup_state(data->pinctrl_gpios, "power_off");
			if (IS_ERR(data->pins_power_off)) {
				ret = PTR_ERR(data->pins_power_off);
				devm_pinctrl_put(data->pinctrl_gpios);
				printk(KERN_ERR "%s can't find fingerprint pinctrl pins_power_off\n", __func__);
				return ret;
			}
#endif
...
static void delete_device_node(void)
{
	...
	//spi_clk_enable(0);
#ifdef VCC_SUPPLY_USE_REGULATER
	regulator_put(egistec_regulater);
#endif
	devm_pinctrl_put(g_data->pinctrl_gpios);
...
static int egistec_proc_init(struct egistec_data *data)
{
	DEBUG_PRINT("egistec_proc_init enter\n");
	if (data) {
		data->proc_root = NULL;
...
static int egistec_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", "et520");
	return 0;
}

static int egistec_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, egistec_proc_show, NULL);
}

static const struct file_operations egistec_proc_fops = {
	.owner = THIS_MODULE,
	.read  = seq_read,
	.open  = egistec_proc_open,
	.release = single_release,
};
static void egistec_proc_create_node(struct egistec_data *data)
{
	DEBUG_PRINT("egistec_proc_create_node enter \n");
	if (data->proc_entry == NULL) {
		DEBUG_PRINT("egistec_proc_create_node data_proc_entry \n");
		data->proc_root = NULL;
		data->proc_entry = proc_create(PROC_NODE, 0666, data->proc_root, &egistec_proc_fops);
		if (data->proc_entry == NULL) {
			DEBUG_PRINT("egistec_proc_create_node fail \n");
			remove_proc_entry(PROC_NODE, data->proc_root);
			data->proc_root = NULL;
			data->proc_entry = NULL;
		}
	}
...
static int __init egis512_init(void)
{
	int status = 0;
	 ...
	 pr_warn("egistec_module_init_warn");
#if 0
...
*/
	...
	pr_err("egistec_module_init1");
	pr_warn("egistec_module_init1_warn");
		sim_gpio = of_get_named_gpio(node, "fp-id-gpios", 0);
		pr_err("egistec_module_init2");
	pr_warn("egistec_module_init2_warn");
	sim_val = __gpio_get_value(sim_gpio);
		*/
	//liukangping@huaqin.com add fp id gpio 20180716 end
	pr_err("egistec_module_init3");
	pr_warn("egistec_module_init3_warn");
...
//add for dual sensor end
	...
	pr_err("egistec_module_init4");
	pr_warn("egistec_module_init4_warn");
	status = platform_driver_register(&egistec_driver);
	pr_err("egistec_module_init5");
	 pr_warn("egistec_module_init5_warn");
...
{
	//liukangping@huaqin.con add fingerprint power ldo_vout begin
	int  ldo_vout_status;
	static struct regulator *buck;

	buck = regulator_get(NULL, "vmch");
	if (buck == NULL) {
			printk("egitec enter cdfinger probe regulator_get r \n");
			return -1;
	}
	printk("egitec enter cdfinger probe regulator_get \n");
	ldo_vout_status = regulator_set_voltage(buck, 3300000, 3300000);
	if (ldo_vout_status < 0) {
			printk("egitec enter cdfinger probe regulator_set_voltage fail \n");
			return -1;
	}
	printk("egitec enter cdfinger probe regulator_set_voltage success \n");
	ldo_vout_status = regulator_enable(buck);
	if (ldo_vout_status < 0) {
			printk("egitec enter cdfinger probe regulator_enable r \n");
			return -1;
	}
	printk("liukangping enter cdfinger probe regulator_enable r1111 \n");
return 0 ;
}
