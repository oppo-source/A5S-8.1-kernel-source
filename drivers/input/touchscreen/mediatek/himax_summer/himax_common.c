...
#include <linux/tp_usb_notifier.h>//wdd add for usb AC mode
#endif
...
char *i_CTPM_firmware_name_sort = "Himax_firmware_sort.bin";
...
static void himax_esd_resume_func(void);//wdd add
...
static ssize_t hx_limit_ctrl_write(struct	file *file,	const	char __user	*buffer, size_t	count, loff_t	*ppos)
{
	...
	if(ts->limit_edge == 0)//wdd add oppo cmd 0 mean in horizon direction & always on limit funcation,do not write 0 to close
	{
								  ...
								  size_t len, loff_t *pos)
{
	...
	struct himax_ts_data *ts = private_ts;
	char *temp_buf;
	I("%s: himax_double_tap_en_read in .\n", __func__);

	if ( *pos ) {
		E("is already read,return . \n");
		return 0;
	}
	*pos += len;
    ...
    *pos += len;
	...
	if (buf[0] == '0') {
		...
		if (ts->SMWP_enable == 0 && ts->psensor_stus == 1) {
			...
			if (ts->gesture_cmd_stus == 0){
				g_core_fp.fp_set_SMWP_enable(ts->SMWP_enable, ts->suspended);
			}
			if (ts->suspended == true && ts->gesture_cmd_stus == 0 ){
				ts->gesture_cmd_stus =1;
			}
			I("%s: gesture_cmd_stus = %d.\n", __func__, ts->gesture_cmd_stus);
...
static int himax_touch_get(struct himax_ts_data *ts, uint8_t *buf, int ts_path, int ts_status)
{
	...
	case HX_REPORT_SMWP_EVENT:
		//g_core_fp.fp_burst_enable(0);//wdd modify polling spi ready
		do{
...
GET_TOUCH_FAIL:
	...
	if(ts_path == HX_REPORT_SMWP_EVENT){
		I("%s:Now on gesture mode,no allow reset .\n", __func__);
	}else{
...
FW_force_upgrade:
	...
	INIT_WORK(&ts->resume_work_queue, himax_resume_workqueue);//wdd add
...
/*check zero flash status*/
	...
	for(i=0 ;i<3 ;i++) {
		...
		if ((data[1] == 0x72 && data[0] == 0xC0)) {
			...
			err_cnt++;
			msleep(10);
			I("[HIMAX TP MSG]: Status check - ALL Zero in ZF. times=%d\n",err_cnt);
		}
	}
