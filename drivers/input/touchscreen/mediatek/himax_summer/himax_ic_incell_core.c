...
#ifdef CORE_IC
/* IC side start*/
static int himax_mcu_burst_enable(uint8_t auto_add_4_byte)//wdd modify return value for spi fail
{
	int	ret = 0;
...
int himax_zf_part_info(const struct firmware *fw_entry)
{
	...
	int cfg_sz = 0;
	int retry = 3;
	uint8_t data_flash[FOUR_BYTE_DATA_SZ];
	...
	if (G_POWERONOF == 1) {
		...
		if (himax_sram_write_crc_check(fw_entry, zf_info_arr[0].sram_addr, zf_info_arr[0].fw_addr, zf_info_arr[0].write_size) != 0) {
			...
			data_flash[2] = 0;
			data_flash[3] = 0;
			g_core_fp.fp_register_write(pdriver_op->addr_fw_define_2nd_flash_reload, FOUR_BYTE_DATA_SZ, data_flash, 0);
			I("HW CRC FAIL ,AND CLEAR flash_reload \n");
		} else {
			I("%s, HW CRC PASS\n", __func__);
		}
		...
		if (cfg_crc_hw != cfg_crc_sw) {
			...
			data_flash[3] = 0;
			g_core_fp.fp_register_write(pdriver_op->addr_fw_define_2nd_flash_reload, FOUR_BYTE_DATA_SZ, data_flash, 0);
			I("Config CRC FAIL ,AND CLEAR flash_reload \n");
		}
   } else {
...
static int himax_mcu_0f_esd_check(void)
{
	...
	//Bin.Su@ODM_WT.BSP.TP.FUNCTION.2019/04/11, below register is useless for 83112c
...
static void himax_mcu_fp_init(void)
{
	...
	g_core_fp.fp_ultra_enter = himax_mcu_ultra_enter;//wdd add for psensor func
...
void himax_mcu_in_cmd_init(void)
{
	...
	himax_in_parse_assign_cmd(fw_oppo_tp_direction, pfw_op->addr_oppo_tp_direction, sizeof(pfw_op->addr_oppo_tp_direction));//wdd add
