...
static int himax_common_probe_spi(struct spi_device *spi)
{
	...
	mutex_init(&ts->resume_update_fw_lock);//wdd add
...
//struct pinctrl_state *spi_cs, *spi_clk, *spi_mo, *spi_mi;
...
static int himax_common_local_init(void) {
	...
	himax_spi_pinctrl = devm_pinctrl_get(tpd->tpd_dev);
	if (IS_ERR(himax_spi_pinctrl)) {
		ret = PTR_ERR(himax_spi_pinctrl);
		E("Cannot find touch spi pinctrl!\n");
	}

/*	spi_cs = pinctrl_lookup_state(himax_spi_pinctrl, "state_spi_cs");
	if (IS_ERR(spi_cs)) {
		ret = PTR_ERR(spi_cs);
		E("Cannot find hxtp_cs_on  %d!\n", ret);
	} else {
		pinctrl_select_state(himax_spi_pinctrl, spi_cs);
	}

	spi_clk = pinctrl_lookup_state(himax_spi_pinctrl, "state_spi_clk");
	if (IS_ERR(spi_clk)) {
		ret = PTR_ERR(spi_clk);
		E("Cannot find hxtp_clk_on %d!\n", ret);
	} else {
		pinctrl_select_state(himax_spi_pinctrl, spi_clk);
	}

	spi_mi = pinctrl_lookup_state(himax_spi_pinctrl, "state_spi_mi");
	if (IS_ERR(spi_mi)) {
		ret = PTR_ERR(spi_mi);
		E("Cannot find hxtp_mi_on %d!\n", ret);
	} else {
		pinctrl_select_state(himax_spi_pinctrl, spi_mi);
	}

	spi_mo = pinctrl_lookup_state(himax_spi_pinctrl, "state_spi_mo");
	if (IS_ERR(spi_mo)) {
		ret = PTR_ERR(spi_mo);
		E("Cannot find hxtp_mo_on %d!\n", ret);
	} else {
		pinctrl_select_state(himax_spi_pinctrl, spi_mo);
	} */
