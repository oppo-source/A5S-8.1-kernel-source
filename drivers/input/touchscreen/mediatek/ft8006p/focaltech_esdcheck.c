...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    FTS_DEBUG("[ESD]: NoACK=%d, Error Data=%d, Hardware Reset=%d", fts_esdcheck_data.nack_cnt, fts_esdcheck_data.dataerror_cnt, fts_esdcheck_data.hardware_reset_cnt);
    return 0;
}
...
int fts_create_esd_sysfs(struct device *dev)
{
    ...
    if (ret) {
        FTS_ERROR( "esd sys node create fail");
