...
static int ft8006p_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    ...
    /* compare */
    if (tdata->in_blackscreen) {
        tmp_result = compare_array(rawdata,
                                   thr->lpwg_rawdata_min,
                                   thr->lpwg_rawdata_max,
                                   false);
    } else {
        tmp_result = compare_array(rawdata,
                                   thr->rawdata_min,
...
static int ft8006p_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
    ...
    if (tdata->in_blackscreen)
        frame_num = thr->basic.lpwg_lcdnoise_frame;
    else
    ...
    /* compare */
    if (tdata->in_blackscreen)
        lc = thr->basic.lpwg_lcdnoise_coefficient;
    else 
        lc = thr->basic.lcdnoise_coefficient;
    max = lc * touch_thr * 32 / 100;
...
static int param_init_ft8006p(void)
{
    struct incell_threshold *thr = &fts_ftest->ic.incell.thr;

    get_value_basic("LPWG_RawDataTest_Min", &thr->basic.lpwg_rawdata_min);
    get_value_basic("LPWG_RawDataTest_Max", &thr->basic.lpwg_rawdata_max);

    get_value_basic("LPWG_NoiseTest_Frames", &thr->basic.lpwg_lcdnoise_frame);
    get_value_basic("LPWG_NoiseTest_Coefficient", &thr->basic.lpwg_lcdnoise_coefficient);
    get_value_basic("LPWG_NoiseTest_Coefficient_Key", &thr->basic.lpwg_lcdnoise_coefficient_vkey);
