...
void print_buffer(int *buffer, int length, int line_num)
{
	...
	return; /* print will cause hangup */
...
/********************************************************************
 * test spi read/write interface
 *******************************************************************/
static int fts_test_spi_read(u8 *writebuf, int writelen, u8 *readbuf, int readlen)
{
    int ret = 0;

    ret = fts_read(writebuf, writelen, readbuf, readlen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
static int fts_test_spi_write(u8 *writebuf, int writelen)
{
    int ret = 0;
...
{
    return fts_test_spi_read(&addr, 1, val, 1);
}
...
int fts_test_read(u8 addr, u8 *readbuf, int readlen)
{
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_test_spi_read(&addr, 1, &readbuf[offset], packet_length);
        if (ret < 0) {
...
int fts_test_write(u8 addr, u8 *writebuf, int writelen)
{
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_test_spi_write(data, packet_length + 1);
        if (ret < 0) {
...
int read_mass_data(u8 addr, int byte_num, int *buf)
{
    ...
    for (i = 0; i < byte_num; i = i + 2) {
        buf[i >> 1] = (int)((((s16)((s16)data[i]) << 8) + (s16)data[i + 1]));
		if (buf[i >> 1] > 60000) {
			buf[i >> 1] -= 65536;
		}
    }
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
	...
	FTS_INFO("save file name:%s", filepath);
    if (NULL == pfile) {
        pfile = filp_open(filepath, O_TRUNC | O_CREAT | O_RDWR, 644);
    }
    if (IS_ERR(pfile)) {
...
static int fts_test_malloc_free_incell(struct fts_test *tdata, bool allocate)
{
    ...
    if (true == allocate) {
        ...
        fts_malloc_r(thr->lpwg_rawdata_min, buflen);
        fts_malloc_r(thr->lpwg_rawdata_max, buflen);
    } else {
        ...
        fts_free(thr->cb_min);
        fts_free(thr->cb_max);
        fts_free(thr->lpwg_rawdata_min);
        fts_free(thr->lpwg_rawdata_max);
    }
...
static int fts_test_oppo_entry(void)
{
	char ini_file[128] = { 0 };
	snprintf(ini_file, 128, "%s", "test_config.ini");

	return fts_test_entry(ini_file);
}
...
int fts_oppo_test(bool in_blackscreen, int *test_result)
{
	...
	fts_ftest->in_blackscreen = in_blackscreen;
	*test_result = -1;
	ret = fts_fw_enter_test_environment(1);
	if (ret < 0) {
		FTS_ERROR("enter test environment fail");
	} else {
		ret = fts_test_oppo_entry();
		if (ret < 0) {
			FTS_TEST_ERROR("oppo test fail");
			*test_result = -2;
		} else {
			*test_result = 0;
		}
...
{   
	...
	rawdata = (int *)kzalloc(tdata->node.node_num * sizeof(int), GFP_KERNEL);
	if (NULL == rawdata) {
		FTS_TEST_ERROR("rawdata zalloc fail");
		return -ENOMEM;
	}
    ...
    ret = get_rawdata(rawdata);
	if (ret < 0) {
		FTS_TEST_ERROR("get rawdata fail");
		goto enter_work;
	}
	
	for (i = 0; i < tdata->node.node_num; i++) {
		len += snprintf(buf + len, *buflen - len, "%5d,", rawdata[i]);
		if ((0 == (i + 1) % tdata->node.rx_num))
...
/* oppo need test in suspend blck scren
...
//    fts_ftest->in_blackscreen = true;
    ...
    ret = fts_fw_enter_test_environment(0);
    if (ret < 0) {
...
int fts_test_init(struct device *dev)
{
    ...
    ret = sysfs_create_group(&dev->kobj, &fts_test_attribute_group);
    if (0 != ret) {
        FTS_TEST_ERROR( "[focal] %s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(&dev->kobj, &fts_test_attribute_group);
    } else {
