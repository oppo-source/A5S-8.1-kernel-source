...
void dump_data1(void *data, int type, int len, int row_len, const char *name)
{
	int i, row = 31;
	uint8_t *p8 = NULL;
	int32_t *p32 = NULL;
...
static void mp_compare_cdc_result(int test_item,int32_t *tmp, int32_t *max_ts, int32_t *min_ts, int* result)
{
	int x, y;
...
int core_mp_compare_retry_cdc_result(int i)
{
	...
	if (tItems[i].catalog == TX_RX_DELTA) {
		...
		mp_compare_cdc_result(tItems[i].catalog, core_mp->tx_max_buf, max_threshold, min_threshold, &test_result);
		mp_compare_cdc_result(tItems[i].catalog, core_mp->tx_min_buf, max_threshold, min_threshold, &test_result);
		for(j = 0 ;j < core_mp->frame_len ; j++) {
			max_threshold[j] = core_mp->RxDeltaMax;
			min_threshold[j] = core_mp->RxDeltaMin;
		}
		mp_compare_cdc_result(tItems[i].catalog, core_mp->rx_max_buf, max_threshold, min_threshold, &test_result);
		mp_compare_cdc_result(tItems[i].catalog, core_mp->rx_min_buf, max_threshold, min_threshold, &test_result);
	} else {
		...
		if(tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST)
		{
			...
			mp_compare_cdc_result(tItems[i].catalog, tItems[i].result_buf, max_threshold, min_threshold, &test_result);
		}
		else
		{
			mp_compare_cdc_result(tItems[i].catalog, tItems[i].max_buf, max_threshold, min_threshold, &test_result);
			mp_compare_cdc_result(tItems[i].catalog, tItems[i].min_buf, max_threshold, min_threshold, &test_result);
		}
	}
...
//Bin.Su@ODM_WT.BSP.TP.FUNCTION.2018/11/14,remove no useless codes for open short test
	...
	ilitek_platform_disable_irq();
	ipio_info("retry test Item begin\n");
...
int core_mp_move_code(void)
{
	...
	if(res < 0)
		ipio_err("core_mp_move_code failed");
#else
	if (core_config_check_cdc_busy(50, 50) < 0) {
